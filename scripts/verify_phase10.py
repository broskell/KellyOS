...
OUT = Path("docs/handoffs/phase10-verify")
OUT.mkdir(parents=True, exist_ok=True)
BASE = os.environ.get("KELLOS_PREVIEW", "http://127.0.0.1:4173")
LIVE = os.environ.get("KELLOS_LIVE", "")
...
def assert_meta(html: str, title_fragment: str):
    assert title_fragment in html
    assert 'rel="canonical"' in html
    assert 'property="og:image"' in html
    assert "/og.png" in html
    assert "KELL.OS 1.0" in html
    assert "<title>ORIGIN" not in html
    assert UNVERIFIED_PR not in html
    assert "roast-my-project.vercel.app" not in html.split("<body")[0]


def check_nojs_and_meta(browser, origin: str, label: str):
    ctx0 = browser.new_context(java_script_enabled=False, viewport={"width": 1280, "height": 800})
    page = ctx0.new_page()
    page.goto(f"{origin}/", wait_until="domcontentloaded")
    html = page.content()
    assert "I develop AI-assisted" in html, f"{label} no-JS home missing disclosure"
    assert_meta(html, "KELL.OS 1.0 — Saathvik Kellampalli")
    page.goto(f"{origin}/now", wait_until="domcontentloaded")
    assert "I'm actually stuck on" in page.content()
    assert "August 2026" in page.content()
    page.goto(f"{origin}/timeline", wait_until="domcontentloaded")
    assert "CGPA 9.44" in page.content()
    page.goto(f"{origin}/project/langchain-openrouter-provider", wait_until="domcontentloaded")
    html = page.content()
    assert "And the part I won't dress up" in html, f"{label} nested prerender missing case study"
    assert UNVERIFIED_PR not in html
    assert UNVERIFIED_ISSUE not in html
    assert "Outbound links are not published until they have been verified." in html
    assert_meta(html, "Landing a feature in LangChain")
    page.goto(f"{origin}/read/about", wait_until="domcontentloaded")
    html = page.content()
    assert "I develop AI-assisted" in html
    assert_meta(html, "Reader Mode")
    page.goto(f"{origin}/projects", wait_until="domcontentloaded")
    html = page.content()
    assert "Roast My Project" in html
    assert "PawSethu" in html
    page.goto(f"{origin}/og.png", wait_until="domcontentloaded")
    assert page.url.endswith("/og.png") or "og.png" in page.url
    ctx0.close()


def check_js_path(browser, origin: str, label: str, prefix: str):
    ...
    for dead in ("Terminal", "KELL.AI", "Search", "Settings", "OS Update"):
        assert start.get_by_text(dead, exact=True).count() == 0, f"{label} Start lists {dead}"
    ...
    shot(page, f"{prefix}-gallery.png")
    ...
    shot(page, f"{prefix}-90s-path.png")
    ...
    shot(page, f"{prefix}-reader-alt-r.png")
    ctx.close()
    ...
    assert pos != "absolute", f"{label} mobile WM overlapping via absolute: {pos}"
    shot(page, f"{prefix}-mobile.png")
    ctxm.close()


with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    check_nojs_and_meta(browser, BASE, "preview")
    check_js_path(browser, BASE, "preview", "preview")
    live_ok = False
    if LIVE:
        check_nojs_and_meta(browser, LIVE.rstrip("/"), "live")
        check_js_path(browser, LIVE.rstrip("/"), "live", "live")
        live_ok = True
    browser.close()

(OUT / "report.json").write_text(
    f'{{"ok": true, "preview": "{BASE}", "live": "{LIVE}", "liveVerified": {str(live_ok).lower()}}}\n',
    encoding="utf8",
)
print("phase10 verify ok" + (" (preview + live)" if live_ok else " (preview only)"))
sys.exit(0)
