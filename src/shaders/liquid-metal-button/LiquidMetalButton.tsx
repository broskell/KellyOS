...
import rawSource from "./liquid-metal-button.html?raw";

const liquidMetalButtonSource =
  typeof rawSource === "string" ? rawSource : (rawSource as any)?.default || "";
...
<script id="liquid-metal-button-bridge">
  ...
  btn.addEventListener('click', () => {
    window.parent.postMessage({ liquidMetalButton: { type: 'activate' } }, '*');
  });
</script>`;
...
export function LiquidMetalButton({
  ...
  const safeText = String(text ?? (safeVariant === "pill" ? "Sign up" : safeVariant === "circle" ? "Add" : "Play"))
    .slice(0, 24);
  const buttonHeight = safeVariant === "pill" ? (height ?? 40) : diameter;
  const pillWidthUnits = safeVariant === "pill"
    ? Math.min(3000, Math.max(1407, 820 + safeText.length * 94))
    : undefined;
  const widthPx = safeVariant === "pill"
    ? Math.ceil((pillWidthUnits || 1407) * (buttonHeight / 516))
    : buttonHeight;
  ...
  const syncButtonConfig = useCallback(() => {
    frameRef.current?.contentWindow?.postMessage({
      liquidMetalButton: { text: safeText, pillWidthUnits, embedded, height: buttonHeight },
    }, "*");
  }, [buttonHeight, embedded, pillWidthUnits, safeText]);
  ...
  }, [mounted, syncButtonConfig, syncPlayConfig]);
  ...
  const clipRadius = safeVariant === "pill" ? "999px" : "50%";
