...
export interface ImageData {
  ...
  description?: string;
  /**
   * Optional. Shown if `src` fails to load (e.g. an offline/blocked CDN logo).
   * A data-URI is ideal so the fallback itself never hits the network.
   */
  fallbackSrc?: string;
}
...
export interface SphereImageGridProps {
  ...
  className?: string;
  /**
   * Optional. When provided, clicking a node calls this instead of opening the
   * built-in enlarged-image modal — letting a parent drive its own detail UI
   * (e.g. a skill panel). Leave undefined to keep the default modal behavior.
   */
  onSelect?: (image: ImageData) => void;
  /**
   * Optional. Render `<img loading="eager">` on the sprites — the CDN logos this
   * portfolio uses are small and better fetched up front so the sphere never
   * shows gaps while spinning. Defaults to the original lazy-after-3 behavior.
   */
  eagerImages?: boolean;
}
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  maxRotationSpeed = 5,
  baseImageScale = 0.12,
  // hoverScale is part of the public props but the render uses a fixed 1.2×
  // hover pop; kept in the interface for API compatibility.
  perspective = 1000,
  autoRotate = false,
  autoRotateSpeed = 0.3,
  className = '',
  ...
  const handleNodeActivate = useCallback((image: ImageData) => {
    if (onSelect) {
      onSelect(image);
    } else {
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      <div
        ...
        onClick={() => handleNodeActivate(image)}
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/20">
          <img
            ...
            draggable={false}
            loading={eagerImages || index < 3 ? 'eager' : 'lazy'}
            onError={(e) => {
              const img = e.currentTarget;
              if (image.fallbackSrc && img.src !== image.fallbackSrc) {
  ...
  }, [worldPositions, baseImageSize, containerSize, hoveredIndex, handleNodeActivate, eagerImages]);
