...
const drawLine = (
  ctx: CanvasRenderingContext2D,
  line: string,
  /** Center x — each line is centered around it. */
  x: number,
  y: number,
  letterSpacing: number,
) => {
...
const buildTextCanvas = ({
  ...
  if (!Number.isFinite(lineHeight)) {
    lineHeight =
      fontSizePx * (typeof props.lineHeight === "number" ? props.lineHeight : 0.92);
  }
  probe.remove();
...
const WarpText = ({
  ...
  const contextRef = useRef<{ program: unknown; rasterize: () => void } | null>(null);
