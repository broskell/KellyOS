...
/**
 * Beams — a field of noise-displaced light beams (React-Three-Fiber). Used as the
 * ambient backdrop of the Contact section in the 26' system. Faithful TS port of
 * the React-Bits Beams component; the WebGL/shader logic is unchanged.
 *
 * Kept monochrome for the 26' site: a white directional light rakes across
 * near-black beams, so the result reads as soft grey light streaks over the page
 * canvas. `backgroundColor` matches the page canvas (#070707) so the Canvas blends
 * seamlessly into the section rather than painting a hard black box.
 */
...
type MaterialCtor = new (params?: Record<string, unknown>) => THREE.Material &
  Record<string, unknown>;

function extendMaterial(BaseMaterial: MaterialCtor, cfg: ExtendMaterialConfig) {
  ...
  const defaults = new BaseMaterial(cfg.material || {}) as Record<string, unknown> & {
    color?: THREE.Color;
  };
...
const Beams = ({
  ...
  const beamMaterial = useMemo(
    () =>
      extendMaterial(THREE.MeshStandardMaterial as unknown as MaterialCtor, {
        header: `
  varying vec3 vEye;
